function [H] = histogram_smoothing(H, KDthreshold, smoothing)
    ...
    [bandwidth,density,X,Y]=kde2d([idx H]);
    ...
    %%%% Surface plot of the kernel density estimation of the data points on
    %%%% the histogram %%%%
    % figure
    % surf(X(50:200,:),Y(50:200,:),density(50:200,:))
    % colormap('jet')
    ...
    H = smoothts(interp1(locations, H(locations), 1:length(H), 'pchip'), 'e', smoothing); 
    H = [idx H'];    
end    